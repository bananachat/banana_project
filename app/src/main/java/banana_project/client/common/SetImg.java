package banana_project.client.common;

import javax.swing.*;

public class SetImg {
    // 이미지 경로
    public static final String imgPath = "./app\\src\\main\\java\\banana_project\\image\\";

    // 로고 이미지
    // 타이틀창 아이콘 이미지
    public static final ImageIcon img_title = new ImageIcon(imgPath + "logo_title.png");
    // 메인 로고 이미지
    public static final ImageIcon img_main = new ImageIcon(imgPath + "logo_main.png");
    // 찾기 로고 이미지
    public static final ImageIcon img_find = new ImageIcon(imgPath + "logo_find.png");
    // 회원가입 로고 이미지
    public static final ImageIcon img_join = new ImageIcon(imgPath + "logo_join.png");

    // 채팅방 이미지
    // 채팅방 돌아가기 버튼 이미지
    public static final ImageIcon img_backbtn = new ImageIcon(imgPath + "btn_back.png");
    // 채팅방 바탕화면 이미지
    public static final ImageIcon img_background = new ImageIcon(imgPath + "bg_room.png");

    // JOption 아이콘
    // JOp 인포 이미지
    public static final ImageIcon img_info = new ImageIcon(imgPath + "mini_info.png");
    // JOp 확인 이미지
    public static final ImageIcon img_confirm = new ImageIcon(imgPath + "mini_confirm.png");
    // JOp 취소 이미지
    public static final ImageIcon img_notFound = new ImageIcon(imgPath + "mini_notFound.png");
    // JOp 추가 이미지
    public static final ImageIcon img_add = new ImageIcon(imgPath + "mini_add.png");
    // JOp 삭제 이미지
    public static final ImageIcon img_delete = new ImageIcon(imgPath + "mini_delete.png");
    // JOp 존재 이미지
    public static final ImageIcon img_exist = new ImageIcon(imgPath + "mini_exist.png");
    // JOp 계정존재 이미지
    public static final ImageIcon img_existAcnt = new ImageIcon(imgPath + "mini_exist_account.png");
}
